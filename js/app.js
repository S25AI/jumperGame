...
class Monster {
  ...
  update(time, state) {
    ...
    let posY = newPos.y + this.size.y;
    let posXLeft = Math.floor(newPos.x);
    let posXRight = Math.floor(newPos.x + this.size.x)

    let isNearCliff = [rows[posY][posXLeft], rows[posY][posXRight]].find(ch => ch == 'empty'); 

    if (!state.level.touches(newPos, this.size, "wall") && !isNearCliff) {
  ...
  collide(state) {
    let {player} = state;
    let playerBottomYCoord = player.pos.y + player.size.y;
    let topMonsterCoord = this.pos.y;

    if (playerBottomYCoord <= topMonsterCoord + 0.2) {
  ...
  static create(pos) {
    return new Monster(pos.plus(new Vec(0, -1.5)), new Vec(1, 0));
  }

  get size() {
    return new Vec(1.5, 2.5);
  }
...
function elt(node, attrs, ...children) {
